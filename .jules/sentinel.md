...
## 2026-04-17 - [HIGH] Removed unsafe-inline from where_in_the_world script-src CSP
**Vulnerability:** The 'where_in_the_world' application's Content Security Policy included 'unsafe-inline' in the 'script-src' directive, unnecessarily exposing the application to XSS attacks via injected inline scripts.
**Learning:** Including 'unsafe-inline' in `script-src` largely defeats the purpose of a CSP. While some libraries like Leaflet may require `unsafe-inline` in `style-src` to render components dynamically, `script-src` should strictly enforce loading from secure, trusted origins without inline exceptions.
**Prevention:** Remove 'unsafe-inline' from 'script-src' in the CSP headers to enforce a strict security policy against unauthorized inline script execution, while preserving it only where technically necessary (e.g., `style-src` for maps).
## 2026-04-18 - [MEDIUM] Added Input Length Limits to ID lookups in Flask Apps
**Vulnerability:** Several Flask endpoints (`mcq_flashcards`, `sliding_rows_flashcards`, `trivia_flashcards`, and `where_in_the_world`) accepted string parameters via JSON payloads (e.g., `id`, `chain_id`, `target_id`) and immediately used them in dictionary lookups. While type validation (`isinstance(id, str)`) was present, there was no boundary limit on the string length. An attacker could send excessively large strings (e.g., a 10MB string, bypassing `MAX_CONTENT_LENGTH` logic depending on exact config timing) leading to memory exhaustion and hash computation overhead during dictionary lookups.
**Learning:** Checking for string types is not enough; strings must also be length-validated to prevent DoS via excessive memory allocation and processing overhead, especially when used in backend mapping operations.
**Prevention:** Always enforce strict length limits (e.g., `len(id) > 50` or `100`) on string parameters received from untrusted JSON payloads before using them in application logic.
## 2026-05-20 - [MEDIUM] Fix Unhandled TypeErrors and DoS from Unbounded String Length in JSON Payloads
**Vulnerability:** The `/api/connect` endpoint in `whovian_degrees/app.py` extracted `startActor` and `targetDoctor` directly from the user-provided JSON payload and called `.strip()` on them without validating their type or length.
**Learning:** `request.json` can contain any valid JSON type (list, dictionary, boolean, string). Calling string-specific methods like `.strip()` on non-string inputs results in unhandled `AttributeError` exceptions, causing internal server errors (500) and potential stack trace leakage. Furthermore, unbounded string lengths from user inputs can lead to Denial of Service (DoS) attacks via excessive memory allocation or bloated payloads forwarded to third-party APIs (like Gemini).
**Prevention:** Always validate that values parsed from untrusted JSON payloads strictly match expected types (`isinstance(value, str)`) and enforce specific length boundaries (`len(value) <= 100`) before calling type-specific methods or forwarding the data.
## 2026-04-22 - Prevented External API DoS and Information Disclosure
**Vulnerability:** External API requests using `urllib.request.urlopen()` lacked a timeout, potentially hanging the server, and the catch-all exception block returned `str(e)` directly to the client, exposing internal implementation details.
**Learning:** Relying on default network behaviors can lead to Denial of Service via resource exhaustion, and returning raw exception strings violates the principle of failing securely.
**Prevention:** Always explicitly configure a `timeout` parameter for external network requests and ensure catch-all exception blocks log actual errors internally while returning safe, generic messages to clients.
## 2026-05-24 - [HIGH] Missing Rate Limiting on External AI Endpoints
**Vulnerability:** The `/api/connect` endpoint in `whovian_degrees/app.py` made requests to a paid third-party AI service (Gemini API) using user-supplied inputs without any rate limiting. An attacker could exploit this unauthenticated endpoint to issue an excessive number of requests, resulting in a Denial of Service (DoS) and severe financial exhaustion (Billing DoS).
**Learning:** Endpoints that proxy requests to paid or rate-limited external APIs are highly sensitive and require immediate protection. Relying solely on the third-party's quota limits is insufficient and financially risky. Also, trusting `X-Forwarded-For` for IP-based rate-limiting enables trivial bypass via spoofing. Further, iterating an entire tracked IP dictionary for cleanup on every request introduces a new CPU exhaustion DoS.
**Prevention:** Always implement explicit rate limiting on endpoints interacting with paid APIs using `request.remote_addr` (relying on `ProxyFix` if behind a reverse proxy). Prevent CPU DoS by using lazy, per-IP cleanup during limit checks instead of O(N) full-dictionary iteration per request.
## 2026-05-25 - [MEDIUM] Prevent CPU Exhaustion DoS in Integer Casting
**Vulnerability:** The `/api/questions` endpoint in `where_in_the_world/app.py` extracted the `count` parameter from the query string and directly converted it using Python's `int()` function. If an attacker supplied an exceptionally long string of digits (e.g. `?count=9999...` with millions of characters), the integer conversion would consume massive amounts of CPU time (O(N^2) complexity in Python), leading to an algorithmic Denial of Service (DoS) and thread blocking.
**Learning:** Python's `int()` function is highly inefficient at parsing extremely large strings. Untrusted string inputs must always be bounded by length before attempting type conversion, even if a `try/except ValueError` block is present, as the exception will only trigger after the excessive CPU computation or memory limit is hit.
**Prevention:** Enforce a strict length limit (e.g. `if len(count_str) > 10:`) on numerical string inputs prior to invoking `int()`, falling back securely if the limit is exceeded.
## 2026-04-26 - [MEDIUM] Added Permissions-Policy to Next.js Headers
**Vulnerability:** The Next.js application was missing the `Permissions-Policy` HTTP header, allowing potentially unrestricted access to sensitive browser features (like camera, microphone, geolocation) by the document or embedded iframes if explicitly requested or exploited.
**Learning:** Enforcing a `Permissions-Policy` restricts the browser features that a web document and its iframes can use. It is a critical layer of defense-in-depth, minimizing the blast radius if an XSS or framing vulnerability is exploited by ensuring attackers cannot access sensitive hardware or APIs.
**Prevention:** Configure the `headers()` function in `next.config.ts` to include a `Permissions-Policy` header that explicitly denies unused sensitive features (e.g., `camera=(), microphone=(), geolocation=(), browsing-topics=()`).
## 2026-06-01 - [MEDIUM] Prevent CPU Exhaustion DoS in Node.js Crypto Hashing
**Vulnerability:** The `/api/cron/route.ts` endpoint verified the authorization header by hashing both the expected and actual headers using `crypto.createHash('sha256')` for a timing-safe comparison. While `crypto.timingSafeEqual` prevents timing attacks, the hashing operation itself is computationally expensive. Passing an extremely long, malicious string as the authorization header would cause excessive CPU consumption, leading to a Denial of Service (DoS) vulnerability.
**Learning:** Cryptographic functions in Node.js run on the main thread or thread pool and can become bottlenecks if inputs are unbounded. Attackers can exploit this by sending massive payloads to exhaust server resources.
**Prevention:** Enforce strict length limits (e.g., `if (actualAuthHeader.length > 200)`) on user-supplied inputs like headers or tokens before passing them to any cryptographic hashing or validation functions.
